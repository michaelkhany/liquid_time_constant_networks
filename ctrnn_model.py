...
import pandas as pd
import os
os.environ['CUDA_VISIBLE_DEVICES'] ='1' #= '-1'  # Run on CPU
...
class multiJointArmData:

    def __init__(self,seq_len=32):
        tf.compat.v1.disable_eager_execution() #To avoid raise RuntimeError("tf.placeholder() is not compatible with " RuntimeError: tf.placeholder() is not compatible with eager execution.

        all_files = sorted([os.path.join("data/multiJointArm",d) for d in os.listdir("data/multiJointArm") if d.endswith(".npy")])
...
class multiJointArmModel:

    def __init__(self,model_type,model_size,sparsity_level=0.5,learning_rate = 0.001):
        self.model_type = model_type
        self.constrain_op = []
        self.sparsity_level = sparsity_level
        self.x = tf.compat.v1.compat.v1.placeholder(dtype=tf.compat.v1.float32,shape=[None,None,17])
        self.target_y = tf.compat.v1.compat.v1.placeholder(dtype=tf.compat.v1.float32,shape=[None,None,17])
        ...
        if(model_type == "lstm"):
            ...
            head,_ = tf.compat.v1.nn.dynamic_rnn(self.fused_cell,head,dtype=tf.compat.v1.float32,time_major=True)
            ...
            head,_ = tf.compat.v1.nn.dynamic_rnn(self.wm,head,dtype=tf.compat.v1.float32,time_major=True)
            self.constrain_op.extend(self.wm.get_param_constrain_op())
        elif(model_type == "node"):
            self.fused_cell = ltc.NODE(model_size,cell_clip=-1)
            head,_ = tf.compat.v1.nn.dynamic_rnn(self.fused_cell,head,dtype=tf.compat.v1.float32,time_major=True)
        elif(model_type == "ctgru"):
            self.fused_cell = ltc.CTGRU(model_size,cell_clip=-1)
            head,_ = tf.compat.v1.nn.dynamic_rnn(self.fused_cell,head,dtype=tf.compat.v1.float32,time_major=True)
        elif(model_type == "ctrnn"):
            self.fused_cell = ltc.CTRNN(model_size,cell_clip=-1,global_feedback=True)
            head,_ = tf.compat.v1.nn.dynamic_rnn(self.fused_cell,head,dtype=tf.compat.v1.float32,time_major=True)
        else:
        ...
        self.loss = tf.compat.v1.reduce_mean(tf.compat.v1.square(self.target_y-self.y))
        optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate)
        self.train_step = optimizer.minimize(self.loss)

        self.accuracy = tf.compat.v1.reduce_mean(tf.compat.v1.abs(self.target_y-self.y))

        self.sess = tf.compat.v1.InteractiveSession()
        self.sess.run(tf.compat.v1.global_variables_initializer())

        self.result_file = os.path.join("results","multiJointArm","{}_{}_{:02d}.csv".format(model_type,model_size,int(100*self.sparsity_level)))
        if(not os.path.exists("results/multiJointArm")):
            os.makedirs("results/multiJointArm")
        if(not os.path.isfile(self.result_file)):
            with open(self.result_file,"w") as f:
                f.write("best epoch, train loss, train mae, valid loss, valid mae, test loss, test mae\n")

        self.checkpoint_path = os.path.join("tf.compat.v1_sessions","multiJointArm","{}".format(model_type))
        if(not os.path.exists("tf.compat.v1_sessions/multiJointArm")):
            os.makedirs("tf.compat.v1_sessions/multiJointArm")
            
        self.saver = tf.compat.v1.train.Saver()


    def get_sparsity_ops(self):
        tf.compat.v1_vars = tf.compat.v1.trainable_variables()
        op_list = []
        for v in tf.compat.v1_vars:
    ...
    def fit(self,multiJointArm_data,epochs,verbose=True,log_period=50):
        ...
        for e in range(epochs):
            if(verbose and e%log_period == 0):
                test_acc,test_loss = self.sess.run([self.accuracy,self.loss],{self.x:multiJointArm_data.test_x,self.target_y: multiJointArm_data.test_y})
                valid_acc,valid_loss = self.sess.run([self.accuracy,self.loss],{self.x:multiJointArm_data.valid_x,self.target_y: multiJointArm_data.valid_y})
            ...
            losses = []
            accs = []
            for batch_x,batch_y in multiJointArm_data.iterate_train(batch_size=16):
...
if __name__ == "__main__":
    ...
    # python3 multiJointArm.py --size 32 --model lstm --log 1 --epochs 200
    ...
    traffic_data = multiJointArmData()
    model = multiJointArmModel(model_type = args.model,model_size=args.size,sparsity_level=args.sparsity)
