...
def load_crappy_formated_csv():
    ...
    all_x = []    
    with open("data/USDTRY_Data.csv","r") as f:
        lineno = -1
...
class PredictionModel:
    ...
    def __init__(self,model_type,model_size,learning_rate = 0.001):
        ...
        self.result_file = os.path.join("results","trace","{}_{}.csv".format(model_type,model_size))
        if(not os.path.exists("results/predictor")):
            os.makedirs("results/predictor")
        if(not os.path.isfile(self.result_file)):
            with open(self.result_file,"w") as f:
                f.write("best epoch, train loss, train mae, valid loss, valid mae, test loss, test mae\n")

        self.checkpoint_path = os.path.join("tf_sessions","predictor","{}".format(model_type))
        if(not os.path.exists("tf_sessions/predictor")):
            os.makedirs("tf_sessions/predictor")
    ...
    def fit(self,curr_data,epochs,verbose=True,log_period=50):
        ...
        for e in range(epochs):
            if(verbose and e%log_period == 0):
                ...
                print("curr_data.test_y ",str(curr_data.test_y.shape))
                test_acc,test_loss = self.sess.run([self.accuracy,self.loss],{self.x:curr_data.test_x,self.target_y: curr_data.test_y})
                valid_acc,valid_loss = self.sess.run([self.accuracy,self.loss],{self.x:curr_data.valid_x,self.target_y: curr_data.valid_y})
            ...
            losses = []
            accs = []
            for batch_x,batch_y in curr_data.iterate_train(batch_size=16):
